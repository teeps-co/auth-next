...
import { InitTeepsAuth, SignInWithTeepsAuth } from './instance';
import version from './version';
import { getConfig, getLoginUrl, ConfigParameters } from './config';

let instance: SignInWithTeepsAuth & { sessionCache: SessionCache };

function getInstance(): SignInWithTeepsAuth & { sessionCache: SessionCache } {
  if (instance) {
    return instance;
  }
  instance = _initAuth();
...
export const _initAuth = (params?: ConfigParameters): SignInWithTeepsAuth & { sessionCache: SessionCache } => {
  const { baseConfig, nextConfig } = getConfig(params);

  // Init base layer (with base config)
  const getClient = clientFactory(baseConfig, {
    name: 'teeps-auth-next',
    version
  });
...
export const initTeepsAuth: InitTeepsAuth = (params) => {
  const { ...publicApi } = _initAuth(params);
  return publicApi;
};
