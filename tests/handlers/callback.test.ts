...
import { makeIdToken } from '../teeps-auth-session/fixtures/cert';
import { get, post, toSignedCookieJar } from '../teeps-auth-session/fixtures/helpers';
...
describe('callback handler', () => {
  ...
  test('should validate the issuer', async () => {
    ...
    ).rejects.toThrow('unexpected iss value, expected https://acme.teeps.local/, got: other-issuer');
  });
  ...
  test('should create the session without OIDC claims with api config', async () => {
    ...
    expect(session).toStrictEqual({
      ...
      idToken: makeIdToken({ iss: 'https://acme.teeps.local/' }),
      token_type: 'Bearer',
