...
const createHandlers = (params: ConfigParameters): Handlers => {
  ...
  const getClient = clientFactory(config, { name: 'teeps-auth-next', version });
...
export const setup = async (
  ...
  nock('https://test.eu.teeps.com')
    .get('/.well-known/openid-configuration')
    .reply(200, { ...wellKnown, issuer: 'https://test.eu.teeps.com/', end_session_endpoint: undefined });

  nock('https://test.eu.teeps.com', { allowUnmocked: true }).persist().get('/.well-known/jwks.json').reply(200, jwks);
